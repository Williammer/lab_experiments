...
elements = {}; // binding element container
...
Twine.reset = function(newContext, node) {
  ...
  for (key in elements) {
    if (bindings = (ref = elements[key]) != null ? ref.bindings : void 0) { // el[key].bindings -> bindings
      for (j = 0, len = bindings.length; j < len; j++) {
        obj = bindings[j];
        if (obj.teardown) {
          obj.teardown(); // tear down all binding elements
        }
      }
    }
  }
  elements = {};
  rootContext = newContext; // change new context data
  rootNode = node; // root node --> handle context data keys
...
bind = function(context, node, forceSaveContext) {
  ...
  for (type in ref) {
    binding = ref[type];
    if (!(definition = node.getAttribute(type))) { // iterate supported bind types for the node.
      continue;
    }
    if (!element) {
      element = {
...
Twine.unbind = function(node) { // similar with Twine.reset
  ...
  ref1 = node.children || [];
  for (k = 0, len1 = ref1.length; k < len1; k++) {// deep apply unbind
    childNode = ref1[k];
...
keypathForKey = function(key) {
  ...
  keypath = [];
  ref = key.split('.'); // what delimitator is designed here -> key1.key2.key3... for deep object data.
  for (j = 0, len = ref.length; j < len; j++) {
    key = ref[j];
...
// wrapper for get Context data method
...
fireCustomChangeEvent = function(node) { // ?-- no compatiblity issue?
  var event;
...
Twine.bindingTypes = {
  bind: function(node, context, definition) {
    ...
    if (twoWayBinding) {
      ...
      $(node).on('input keyup change', changeHandler);// watcher
      teardown = function() {
